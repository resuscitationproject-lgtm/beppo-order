(function () {
  ...
  function applyStatus(data) {
    const {status,settings}=data; const remaining=Number(status.remaining); const orderingOpen=["受付中","残りわずか"].includes(status.businessStatus)&&remaining>0; const state=remaining<=0?"売り切れ":orderingOpen&&remaining<=3?"残りわずか":status.businessStatus;
    setText("#header-status",state); setText("#header-stock",`${remaining}食`); setText("#hero-status",state); setText("#hero-stock",`あと${remaining}食`); const headerStock=$("#header-stock"),heroStock=$("#hero-stock");if(headerStock)headerStock.hidden=!orderingOpen;if(heroStock)heroStock.hidden=!orderingOpen;setText("#hero-price",new Intl.NumberFormat("ja-JP",{style:"currency",currency:"JPY",maximumFractionDigits:0}).format(1200));
    ...
    const order=$("#order-button"); const unavailable=!orderingOpen; if(order){order.classList.toggle("disabled",unavailable);order.setAttribute("aria-disabled",String(unavailable));order.href=unavailable?"#status-section":"./order.html";order.textContent=remaining<=0?"本日のカレーは売り切れました":status.businessStatus==="売り切れ"?"売り切れ・オーダーストップ":status.businessStatus==="営業終了"?"本日の受付は終了しました":status.businessStatus==="休業"?"本日はお休みです":status.businessStatus==="準備中"?"ただいま準備中です":"モバイル・オーダー";}
