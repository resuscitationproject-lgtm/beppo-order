(function () {
  ...
  function applyStatus(data) {
    ...
    const order=$("#order-button"); const unavailable=remaining<=0||["営業終了","休業","準備中"].includes(status.businessStatus); if(order){order.classList.toggle("disabled",unavailable);order.setAttribute("aria-disabled",String(unavailable));order.href=unavailable?"#status-section":"./order.html";order.textContent=remaining<=0?"本日のカレーは売り切れました":status.businessStatus==="休業"?"本日はお休みです":"今日のカレーを取り置く";}
